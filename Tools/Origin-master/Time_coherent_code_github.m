...
function [XYZ_M_T,Fid_pts_Median] = Time_coherent_code_github(XYZLeads,Fid_pts,fs)
...
%if (Rpeaks(1,1)<(BLengthD2)) || (Rpeaks(1,2)<(BLengthD2)) || (Rpeaks(1,3)<(BLengthD2))
%    c_int=2;
%else
%    c_int=1;
%end
...
c_int=1;
while (Rpeaks(c_int,1)<(BLengthD2)) || (Rpeaks(c_int,2)<(BLengthD2)) || (Rpeaks(c_int,3)<(BLengthD2))
        c_int=c_int+1;
end
...
for ii=c_int:R_no-c_fin
        ...
        Beats_x_T(:,bct)=XYZ_O(I_dv(ii)-((BLengthD2)-1):I_dv(ii)+(BLengthD2),1);
        Beats_y_T(:,bct)=XYZ_O(I_dv(ii)-((BLengthD2)-1):I_dv(ii)+(BLengthD2),2);
        Beats_z_T(:,bct)=XYZ_O(I_dv(ii)-((BLengthD2)-1):I_dv(ii)+(BLengthD2),3);
